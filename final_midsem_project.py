...
# This provides a preview of the top 20 rows of the DataFrame for us to examine
df1.head(20)
...
#Displaying info to check how many columns were removed
...
#equating num 1 to the imputed column
...
#assigning obj1 to the filled column
...
"""We decided to drop the negative correlation because negative correlations can introduce noise into the analysis. we decided to remove them to ensure a clearer and more straightforward understanding of the data
...
#displaying the selected columns based on their correlation
...
"""***Building and  Single Training Models***"""
...
#making predictions and checking the mean absolute error for the random forest model
...
#making predictions and checking the mean absolute error for the Gradient boosting model
...
#making predictions and checking the mean absolute error for the Xgb model
...
"""***TRAINING MODELS WITH CROSS VALIDATION AND EVALUATION / OPTIMIZATION***"""
...
rf_grid_search = GridSearchCV(RandomForestRegressor(), param_grid, cv=5,n_jobs=-1 ,scoring ='neg_mean_absolute_error')
...
gb_grid_search = GridSearchCV(GradientBoostingRegressor(), gb_param_grid, cv=5, n_jobs=-1,scoring='neg_mean_absolute_error')
...
"""Even though we can see that the random forest model is best performing, we test all three models again with the new data set
...
#we did not repeat correlation here as we had already found our chosen columns with the first data set
...
#scaling for the second dataframe
...
#training second dataset with the random forest model

y_pred2=rf_grid_search.predict(X_scaled)

mean_absolute_error(y,y_pred2)

#training with gradient boosting model

y_pred2=gb_grid_search.predict(X_scaled)

mean_absolute_error(y,y_pred2)

#training with xgb model

y_pred2=xgb_grid_search.predict(X_scaled)

mean_absolute_error(y,y_pred2)

"""After training with the new dataset, we still see that randomforest is our best modelhence we select this model for deployement
